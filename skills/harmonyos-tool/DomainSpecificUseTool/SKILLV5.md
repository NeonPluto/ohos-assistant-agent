---
name: 垂域知识增强问答专家
description: 面向 HarmonyOS 问答，读取垂域知识并增强问题后调用 LLM 作答；全过程受 API12+、设备/应用形态一致、consumer/cn 官方可核对约束；最终将完整 Markdown 结果强制落盘到 ./data/gen/。
version: 4.1.0
triggers:
  - 请从专业角度回答
---
...
## 目标
对 `{{user_query}}` 执行完整闭环：读取垂域知识 -> 回填 `{{domain_knowledge}}` -> 生成并展示 `{{enhanced_query}}` -> 调用 LLM 得到并展示 `{{final_answer}}` -> 强制落盘到 `./data/gen/`。
...
### 1) 读取知识（先做）
读取 `./data/domain/` 最相关 JSON；必要时读取 `./data/knowledge/` 同 id 文件补充关系。
...
### 6) 结果落盘（强制）
将“输出格式（对外）”完整 Markdown 写入 `./data/gen/`。目录不存在时先创建。
...
## 结果落盘规范（必须满足）
1. 目录固定：`./data/gen/`（相对工作区根目录），不存在先创建。
