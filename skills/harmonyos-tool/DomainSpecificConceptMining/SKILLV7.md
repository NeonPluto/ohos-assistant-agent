...
- 垂域归并判定：若文件名与当前分析垂域存在上下文关联、同义词、近义词、语义相近关系，均视为同一垂域索引文件，必须复用并追加，不得创建新的平行索引文件。  
...
4. **索引文件存在性检查必须先执行**：先在 `./data/domain/index` 下按 `domain_en` 精确匹配，再按“上下文关联/上下位/同义词/近义词/语义相近”规则做同垂域归并匹配；命中任一既有文件即视为已存在并复用该文件追加记录，未命中才创建 `./data/domain/index/<domain_en>.json` 并写入首条映射记录。  
