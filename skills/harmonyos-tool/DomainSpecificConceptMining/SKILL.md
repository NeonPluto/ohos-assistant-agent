...
你需要输出一条结构化知识，完成：
- 识别“具体 -> 抽象”语义差距
- 判定关系类型（`概念同一`、`同义关系`、`近义关系`、`语境关联`、`上下位关系`、`语义包含关系`）
- 给出垂域 `domain`
...
仅输出以下 JSON（不要输出额外解释或 Markdown 代码块）：
{
  ...
  "knowledge_graph": {
    ...
    "relations": [
      {"subject": "concrete_term", "predicate": "same_as/alias_of/similar_to/context_related_to/is_a/semantically_includes", "object": "abstract_term"},
...
- 垂域归并判定：若文件名与当前分析垂域存在概念同一、同义关系、近义关系、语境关联、上下位关系、语义包含关系，均视为同一垂域索引文件，必须复用并追加，不得创建新的平行索引文件。  
...
3. `internal_storage.domain_index_file_path`（`./data/domain/index/<domain_en>.json`）用于维护“垂域 -> 结构化文件”映射；每条映射至少包含 `id`、`graph_id`、`domain`、`domain_en`、`data_file_path`、`knowledge_file_path`、`knowledge_sentence`、`relation_type`。  
4. **索引文件存在性检查必须先执行**：先在 `./data/domain/index` 下按 `domain_en` 精确匹配，再按“概念同一/同义关系/近义关系/语境关联/上下位关系/语义包含关系”规则做同垂域归并匹配；命中任一既有文件即视为已存在并复用该文件追加记录，未命中才创建 `./data/domain/index/<domain_en>.json` 并写入首条映射记录。  
5. **同步文件创建规则**：当索引文件不存在并新建时，必须同步确保 `./data/domain/knowledge/<id>.json` 与 `./data/domain/graph/<id>.json` 对应文件可创建并成功写入（若目录不存在需先创建目录）。  
6. **按需读取，禁止全量加载**：落盘前仅允许解析目标垂域对应的索引文件，禁止扫描并加载 `./data/domain/index` 下全部文件内容，以减少不必要的文件内容加载与解析。  
7. `id`、`graph_id`、文件名必须保持一致可追踪。  
8. 任意字段缺失时，优先保证 `domain`、`concept_pairs`、`harmonyos_context`、`harmonyos_constraints.compatibility_checklist`、`knowledge_graph.relations` 完整。  
9. 所有落盘地址必须可用且可落盘，但仅作内部参数，不得在页面或用户可见提示词中展示。
